
set_property PACKAGE_PIN T22 [get_ports LED0]
set_property IOSTANDARD LVCMOS33 [get_ports LED0]

set_property PACKAGE_PIN T21 [get_ports LED1]
set_property IOSTANDARD LVCMOS33 [get_ports LED1]
set_property PACKAGE_PIN U22 [get_ports LED2]
set_property IOSTANDARD LVCMOS33 [get_ports LED2]

set_property PACKAGE_PIN U14 [get_ports LED7]
set_property IOSTANDARD LVCMOS33 [get_ports LED7]
set_property PACKAGE_PIN M15 [get_ports SW7]
set_property IOSTANDARD LVCMOS18 [get_ports SW7] 
set_property PACKAGE_PIN H17 [get_ports SW6]
set_property IOSTANDARD LVCMOS18 [get_ports SW6]

set_property PACKAGE_PIN M19 [get_ports SPI_SCK]
set_property IOSTANDARD LVCMOS18 [get_ports SPI_SCK]
set_property PACKAGE_PIN N19 [get_ports SPI_CS]
set_property IOSTANDARD LVCMOS18 [get_ports SPI_CS]
set_property PACKAGE_PIN P17 [get_ports SPI_MOSI]
set_property IOSTANDARD LVCMOS18 [get_ports SPI_MOSI]
set_property PACKAGE_PIN N22 [get_ports SPI_MISO]
set_property IOSTANDARD LVCMOS18 [get_ports SPI_MISO]


set_property PACKAGE_PIN M21 [get_ports MCK]
set_property IOSTANDARD LVCMOS18 [get_ports MCK]

set_property PACKAGE_PIN J18 [get_ports HSICKA0]
set_property IOSTANDARD LVCMOS18 [get_ports HSICKA0]
set_property PACKAGE_PIN L21 [get_ports HSICKA1]
set_property IOSTANDARD LVCMOS18 [get_ports HSICKA1]

set_property PACKAGE_PIN T16 [get_ports HSIA0]
set_property IOSTANDARD LVCMOS18 [get_ports HSIA0]
set_property PACKAGE_PIN J21 [get_ports HSIA1]
set_property IOSTANDARD LVCMOS18 [get_ports HSIA1]
